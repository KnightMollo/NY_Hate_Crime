...
Furthermore, we discovered that there are almost no great gaps in the filled color of each county, except the one locates around -75 longitude and 43 latitude. In other words, counties with large shift tend to connect with each other in at least one direction. Similar for counties with small shift.Such results seems plausible because people are moving between close counties. When one county start to make some improvements on hate crimes, such as using education to decreasing discrimination and increasing police force, its actions not only reduce the number of incidents in its area but also bring positive changes to its neighborhoods. 
