...
Hate Crimes are categorized into 6 sub-categories: Anti-Religion, Anti-Race, Anti-Sexual-Orientation, Anti-Gender, Anti-Disability and Anti-Age. 

The methods of crime are further categorize into two types: Crime against persons and property crimes. 
The stacked bar chart gives us an overview of the relative frequency of each sub-categories of Hate Crimes happened in NY states from 2010 to 2018.

The most common type of hate crime is Anti-religion, and more than half of this type were conducted against property.Then is the Anti-race, Anti-Sexual-Minority, Anti-Gender and Anti-Disability. The least common type of hate crime is Anti-Age, and it composes of almost crimes against property.
...
After having a general picture of frequencies of different types of hate crimes happened in NY states, we continue to dive in to look at frequencies of sub-categories of each type of hate crimes.
...
g1<- dataset %>% 
  ...
  coord_flip()+
  labs(x= 'Subtype of Anti-Gender',y='Number of incidents', fill= 'Type of Crime')+
  ggtitle('Stacked Bar Chart','Anti-Gender')+
  plot_theme

```

We also used stacked bar chart here to not only indicate the counts of subcatories of Anti-Gender but also demonstate the its division in crimes against person or property. Similar code was applied to other types of hate crimes. Eventually, we gained 6 stacked bar charts. 
...
g2<- dataset %>% filter(Type=='Anti.Age') %>% 
  ...
  theme(text = element_text(size=8))+
  coord_flip()+
  labs(x= 'Subtype of Anti-Age',y='Number of incidents', fill= 'Type of Crime')+
...
g3 <- dataset %>% filter(Type=='Anti.Race') %>% 
  ...
  theme(text = element_text(size=8))+
  coord_flip()+
  labs(x= 'Subtype of Anti-Race',y='Number of incidents', fill= 'Type of Crime')+
...
g4 <- dataset %>% filter(Type=='Anti.Religion') %>% ggplot(aes(x=reorder(Subtype,Count),y=Count,fill=Crime.Type))+
  ...
  coord_flip()+
  labs(x= 'Subtype of Anti-Religion',y='Number of incidents', fill= 'Type of Crime')+
...
g5 <- dataset %>% filter(Type=='Anti.Disability') %>% ggplot(aes(x=reorder(Subtype,Count),y=Count,fill=Crime.Type))+
  ...
  theme(text = element_text(size=8))+
  coord_flip()+
  labs(x= 'Subtype of Anti-Disability',y='Number of incidents', fill= 'Type of Crime')+
...
g6 <- dataset %>% filter(Type=='Anti.Sexual.Minority') %>%
  ...
  theme(text = element_text(size=8))+
  coord_flip()+
  labs(x= 'Subtype of Anti-Sexual-Minority',y='Number of incidents', fill= 'Type of Crime')+
...
g6<- g6 + theme(legend.position = "bottom")+ xlab(NULL)
```
...
After investigating the overall distribution of all kinds of hate crimes, we now consider the distribution of different types of hate crimes in different counties. That is, we want to know if for some certain type of hate crimes, the total number of incicents corresponding to it vary in counties or not. Also, it is worth checking whether the distribution in counties changes among various kinds of hate crimes. At this stage, total number of incidents of some type of hate crimes includes the whole time period from 2010 to 2018 and both crime types which are person and property. 

We draw  heatmaps of total number of incidents in New York for different types of hate crimes. Here for simplicity, we used heatmap of Anti-Age as an exmple for code demostration.
...
```{r}
grid.arrange(q1,q2,nrow=2,top= 'New York County Heatmap for different types of hate crimes')
...
plot_grid(p3,p4,nrow=1,ncol=2,labels=c('C','D'))
```
...
dataset %>% filter(Type=='Anti.Gender') %>%
  ...
  geom_line()+
  geom_point()+
  labs(y='Number of Incidents', color='Subtype of AntiGender')+
  ggtitle("Time Series Line Chart","Year VS Subtype of AntiGender")+
...
```{r}
data %>% ggplot(aes(x=long, y = lat, group=group,fill = change_prop)) + 
  geom_polygon(color='white')+
...
Furthermore, we discovered that there are almost no great gaps in the filled color of each county, except the one locates around -75 longitude and 43 latitude. This tells us that neighborhood effect influences the change in the number of total incidents. In other words, counties with large shift tend to connect with each other in at least one direction. Similar for counties with small shift. 
Also, it's easy to see that counties with relatively large shift locates at 43 latitude or at 74 longitude.Thus, from what we got, we may infer that location does matter in the shift of numbers of incidents from 2010 to 2018. 
