...
## Question 1
***
How many hate crimes are taking place in NY state? Who are the people most affected? The bar chart below provides us a clear initial view.
```{r}
...
```{r}
d1 = dataset %>% filter(Type=='Anti.Gender') 
d1$Subtype <- factor(d$Subtype, levels = c("Anti.Gender.Identity.Expression", "Anti.Transgender", "Anti.Female", 	"Anti.Male"))

g1<-  d %>% 
  ggplot(aes(x = fct_rev(Subtype), y = Count, fill = Crime.Type))  + 
  geom_col() +
...
g2<- dataset %>% filter(Type=='Anti.Age') %>% 
  ggplot(aes(x=Subtype,y=Count,fill=Crime.Type))+geom_col()+
  #theme(axis.text.x = element_text(angle = 20, vjust = 0.9))
  theme(text = element_text(size=8))+
  coord_flip()+
   ggtitle("Anti_Age")
...
g3 <- dataset %>% filter(Type=='Anti.Race') %>% 
  ggplot(aes(x= reorder(Subtype, Count, sum),y=Count,fill=Crime.Type))+geom_col()+
  theme(text = element_text(size=8))+
  #theme(axis.text.x = element_text(angle = 20, vjust = 0.9))
  coord_flip()+
   ggtitle("Anti_Race")
...
g4 <- dataset %>% filter(Type=='Anti.Religion') %>% 
  ggplot(aes(x=reorder(Subtype, Count, sum) ,y=Count,fill=Crime.Type))+geom_col()+
...
g5 <- dataset %>% filter(Type=='Anti.Disability') %>% 
  ggplot(aes(x=reorder(Subtype, Count, sum),y=Count,fill=Crime.Type))+geom_col()+
  theme(text = element_text(size=8))+
#theme(axis.text.x = element_text(angle = 20, vjust = 0.9))
  coord_flip()+
...
title <- ggdraw() + draw_label("Hate Crime Incidents by Sub-category")
...
plot_grid(title, g1,g2, nrow = 3,align = "hv",axis = 'l', rel_heights = c(0.1, 0.45, 0.45))
plot_grid(g3,g4, nrow = 2,align = "hv",axis = 'l' )
plot_grid(g5,g6, legend, nrow = 3,align = "hv",axis = 'l' , rel_heights = c(0.45, 0.45, 0.1))
```
...
tidy <- read.csv("Hate_Crimes_data_Tidy.csv")
data2 <- tidy %>% select(County,Type,Count) %>%
       group_by(County,Type)%>%
       summarise(Count = sum(Count))%>%
       mutate(County= tolower(County)) 
data<-full_join(data1,data2,by='County')
q1 <- data%>%filter(Type== 'Anti.Age') %>% ggplot(aes(x=long,y=lat,group=group,fill=Count))+
...
```{r Line Chart: Time Vs Crime Type1, message=FALSE, warning=FALSE}
#manipulate the data to make it suitable for time series line chart
Crime_type<-tidy %>% select(Year,Crime.Type,Count) %>%
...
Type<-tidy %>% select(Year,Type,Count) %>% 
  group_by(Year,Type) %>% 
  summarise(Count=sum(Count)) %>% 
