...
g1<- dataset %>% filter(Type=='Anti.Gender') %>% 
  ggplot(aes(x= fct_infreq(Subtype), y = Count,fill=Crime.Type))+
  geom_col()+
  #theme(axis.text.x = element_text(angle = 20, vjust = 0.9))+
...
g3 <- dataset %>% filter(Type=='Anti.Race') %>% 
  ggplot(aes(x=fct_infreq(Subtype),y=Count,fill=Crime.Type))+geom_col()+
  theme(text = element_text(size=8))+
  #theme(axis.text.x = element_text(angle = 20, vjust = 0.9))
  coord_flip()+
   ggtitle("Anti_Race")
...
g4 <- dataset %>% filter(Type=='Anti.Religion') %>% ggplot(aes(x=Subtype,y=Count,fill=Crime.Type))+geom_col()+
...
g5 <- dataset %>% filter(Type=='Anti.Disability') %>% ggplot(aes(x=Subtype,y=Count,fill=Crime.Type))+geom_col()+
  theme(text = element_text(size=8))+
#theme(axis.text.x = element_text(angle = 20, vjust = 0.9))
  coord_flip()+
...
q6<- data%>%filter(Type== 'Anti.Sexual.Minority') %>% ggplot(aes(x=long,y=lat,group=group,fill=Count))+
  ...
  scale_fill_viridis_c(direction=-1)+
  theme(panel.background = element_blank(),plot.title = element_text(hjust=0.5))   
grid.arrange(q1, q2, nrow = 2,top='Heatmaps of total incidents of different hate types')
...
```{r Line Chart: Time Vs Crime Type2, message=FALSE}
